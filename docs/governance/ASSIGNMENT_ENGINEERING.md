...
Task types are `implementation`, `docs`, `test`, `refactor`, `content`, `research`, and `quality`. Risk markers default to `local-write`; markers such as `deploy`, `release`, `external-write`, or `secret-access` fail closed against the default route.
